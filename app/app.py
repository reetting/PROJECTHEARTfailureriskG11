...
from train_model import train_all_models, save_model, load_model
...
with tab1:
    ...
    X_input_scaled = pd.DataFrame(
        scaler.transform(X_input), columns=FEATURE_NAMES
    )
    ...
    with col2:
        risk_label = "🔴 HIGH RISK" if proba >= 0.65 else "🟡 MODERATE RISK" if proba >= 0.4 else "🟢 LOW RISK"
        color = "#E74C3C" if proba >= 0.65 else "#F39C12" if proba >= 0.4 else "#27AE60"
        st.markdown(f"""
...
with tab2:
    ...
    model, scaler, X_train, _, _ = get_model()
